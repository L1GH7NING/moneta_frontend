...
const Signup = () => {
  ...
  const googleLogin = useGoogleLogin({
    ...
    onSuccess: async (codeResponse) => {
      if (!codeResponse.code) {
        setMessage("error|Google login failed: No authorization code returned");
        return;
      }
      try {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setMessage("success|Signup successful! You can now login.");
          ...
          {message && (
            <div
              className={`mb-6 p-4 rounded-2xl backdrop-blur-sm flex items-start gap-3 ${
                messageType === "success"
                  ? "bg-green-100 bg-opacity-50 border border-green-200"
                  : "bg-red-100 bg-opacity-50 border border-red-200"
              }`}
            >
              {messageType === "success" ? (
